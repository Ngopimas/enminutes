...
interface InsightDef {
  ...
  render: (p: Product, mins: Record<number, number>, years: number[], lang: 'fr' | 'en') => { title: string; sub: string } | null;
}
...
const insightDefs: InsightDef[] = [
  ...
  },
  {
    ...
    render: (p, mins, years, lang) => {
      ...
      const recentAvg = Math.round(recentMins.reduce((a, b) => a + b, 0) / recentMins.length);
      return lang === 'fr'
        ? {
...
export default function Insights() {
  ...
  const cards = useMemo(() => {
    ...
    }).filter(Boolean) as Array<{ emoji: string; productId: string; title: string; sub: string; product: Product }>;
  }, [lang, salaryRef]);
