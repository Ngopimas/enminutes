  ...
  return (
    <section className="py-12 md:py-16" data-testid="product-explorer">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <p className="text-sm text-muted-foreground mb-6">{t('productExplorerSub')}</p>
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="flex items-center gap-1 shrink-0" data-testid="trend-filter">
            {(['all', 'up', 'down', 'stable'] as const).map(t_ => (
              ...
              >
                {t_ === 'all' ? t('allCategories') : t_ === 'up' ? '↗' : t_ === 'down' ? '↘' : '→'}
              </button>
            ))}
          </div>
        </div>
        ...
        {filtered.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8" data-testid="no-product-found">
            {t('noProductFound')}
          </p>
        ) : (
